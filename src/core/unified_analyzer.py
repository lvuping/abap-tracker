...
from patterns.improved_patterns import ImprovedPatternMatcher, OperationType
...
class UnifiedAnalyzer:
    ...
    def analyze(self, code_snippet: List[str], start_line: int = 0) -> Dict:
        ...
        # SY-UNAME 위치 확인
        success, error_reason = self._verify_seed_location(code_snippet, start_line)
        if not success:
            return self._create_error_result(error_reason)
    ...
    def _verify_seed_location(self, code_snippet: List[str], start_line: int) -> tuple[bool, str]:
        """시드 변수 위치 확인 - returns (success, error_reason)"""
        if start_line >= len(code_snippet):
            error_msg = f"Line {start_line + 1} out of range (file has {len(code_snippet)} lines)"
            self.trace_steps.append(f"❌ {error_msg}")
            return False, error_msg

        target_line = code_snippet[start_line].strip()
        actual_line_content = code_snippet[start_line][:50].strip()  # First 50 chars for display

        # 주석이나 빈 줄 확인
        if not target_line:
            error_msg = f"Line {start_line + 1} is empty"
            self.trace_steps.append(f"❌ {error_msg}")
            return False, error_msg

        if target_line.startswith('*'):
            error_msg = f"Line {start_line + 1} is a comment: '{actual_line_content}'"
            self.trace_steps.append(f"❌ {error_msg}")
            return False, error_msg
        ...
        # Check what's actually at this line
        if 'TYPE' in target_line.upper() and 'SY-UNAME' in target_line.upper():
            error_msg = f"Line {start_line + 1} is a TYPE declaration, not an assignment: '{actual_line_content}'"
        elif 'DATA' in target_line.upper():
            error_msg = f"Line {start_line + 1} is a DATA declaration: '{actual_line_content}'"
        else:
            error_msg = f"No SY-UNAME found at line {start_line + 1}. Found: '{actual_line_content}'"
