...
class CompleteAnalyzer:
    ...
    def find_related_operation(self, code_lines: List[str], target_line: int,
        ...
        if target_line < len(code_lines):
            ...
            # Don't search for operations for simple DATA declarations
            if target_upper.startswith('DATA:') or 'TYPE SY-UNAME' in target_upper:
                # This is just a variable declaration, not a DB operation
                return None, None, None

            # Don't search for operations for structure field definitions
            if 'TYPE' in target_upper and not '=' in target_upper:
                return None, None, None
            ...
            if var_match:
                ...
                # For simple assignments, look for where this variable is actually used
                ...
                for i in range(target_line + 1, min(len(code_lines), target_line + search_radius)):
                    ...
                    for op_name, pattern in self.db_operations.items():
                        ...
                        if match and var_name.upper() in line.upper():
                            ...
                            # Only return if we found a real table
                ...
                # For simple assignments, don't search for unrelated operations
                return None, None, None

        # Only search nearby for operations if it looks like part of a DB statement
        if target_line < len(code_lines):
            target_text = code_lines[target_line]
            # Check if this line is likely part of a database operation
            if not any(kw in target_text.upper() for kw in ['SET', 'WHERE', 'VALUES', 'FROM', 'INTO']):
                # Not part of a DB operation, don't search far
                search_radius = 5

        # Search in expanding radius, but more conservatively
        for radius in [5, 10, 15]:
            ...
            for i in range(max(0, target_line - radius), target_line):
                ...
                if result[0]:
                    # Verify it's really related and not too far
                    if self._is_related_operation(code_lines, i, target_line):
                        return result
    ...
    def _is_related_operation(self, code_lines: List[str], op_line: int,
        ...
        distance = abs(op_line - target_line)

        # Very close lines are likely related
        if distance <= 5:
            return True

        # For medium distance, check if they're in the same logical block
        if distance <= 15:
            ...
            # Track sy-uname flow through variables
            if target_line < len(code_lines):
                target_text = code_lines[target_line]
                # Extract variable name if it's an assignment
                var_match = re.match(r'^\s*(\w+)\s*=.*sy-uname', target_text, re.IGNORECASE)
                if var_match:
                    var_name = var_match.group(1)
                    # Check if the operation line uses this variable
                    if op_line < len(code_lines):
                        op_text = code_lines[op_line]
                        if var_name.upper() in op_text.upper():
                            return True
            ...
            for i in range(start + 1, end):
                if i < len(code_lines):
                    ...
                    if line.startswith(('FORM ', 'ENDFORM', 'METHOD ', 'ENDMETHOD',
                                       'FUNCTION ', 'ENDFUNCTION')):
                        return False
                    # Allow some control structures
                    if line.startswith(('IF ', 'ENDIF', 'LOOP ', 'ENDLOOP')):
                        block_count += 1
                        if block_count > 2:  # Too many control structures
                            return False
