...
from src.insert_values_handler import InsertValuesHandler
from src.multiline_statement_handler import MultilineStatementHandler
from src.complete_analyzer import CompleteAnalyzer
from src.enhanced_insert_handler import EnhancedInsertHandler
from src.enhanced_update_handler import EnhancedUpdateHandler
from src.enhanced_modify_handler import EnhancedModifyHandler
from src.comprehensive_db_handler import ComprehensiveDBHandler
...
class DatabaseOperationsTest:
    ...
    def __init__(self):
        ...
        self.comprehensive_handler = ComprehensiveDBHandler()
    ...
    def define_test_cases(self) -> List[TestCase]:
        ...
        test_cases = [
            # INSERT Tests
            TestCase("T01", "Basic INSERT with structure", "INSERT", "ZTABLE", "CREATED_BY", (16, 20), "structure"),
            TestCase("T02", "INSERT with VALUES clause - single line", "INSERT", "ZTABLE", "USER", (22, 23), "values"),
            TestCase("T03", "INSERT with VALUES - multiple values", "INSERT", "ZTABLE", "USER", (25, 28), "multi_values"),
            TestCase("T04", "INSERT with VALUE constructor", "INSERT", "ZTABLE", "CREATED_BY", (30, 34), "value_constructor"),
            TestCase("T05", "INSERT VALUE constructor multiline", "INSERT", "ZUSER_TABLE", "USERNAME", (36, 43), "value_constructor"),
            TestCase("T06", "INSERT FROM TABLE", "INSERT", "ZTABLE", "CREATED_BY", (45, 48), "from_table"),
            TestCase("T07", "INSERT ACCEPTING DUPLICATE KEYS", "INSERT", "ZTABLE", "CREATED_BY", (50, 51), "accepting_keys"),
            TestCase("T08", "INSERT with field symbols", "INSERT", "ZTABLE", "CREATED_BY", (53, 58), "field_symbol"),
            TestCase("T09", "INSERT with inline declaration", "INSERT", "ZTABLE", "CREATED_BY", (60, 63), "inline"),
            TestCase("T10", "INSERT complex VALUE constructor", "INSERT", "ZCOMPLEX_TABLE", "CREATED_BY", (65, 70), "complex_value"),
            
            # UPDATE Tests
            TestCase("T11", "Basic UPDATE with SET", "UPDATE", "ZTABLE", "CHANGED_BY", (76, 78), "basic_set"),
            TestCase("T12", "UPDATE multiple SET fields", "UPDATE", "ZTABLE", "CHANGED_BY", (80, 84), "multi_set"),
            TestCase("T13", "UPDATE multiline SET", "UPDATE", "ZTABLE", "CHANGED_BY", (86, 92), "multiline_set"),
            TestCase("T14", "UPDATE complex WHERE", "UPDATE", "ZUSER_TABLE", "LAST_MODIFIED_BY", (94, 99), "complex_where"),
            TestCase("T15", "UPDATE FROM structure", "UPDATE", "ZTABLE", "CHANGED_BY", (101, 104), "from_structure"),
            TestCase("T16", "UPDATE with subquery", "UPDATE", "ZTABLE", "PROCESSOR", (106, 109), "subquery"),
            TestCase("T17", "UPDATE CLIENT SPECIFIED", "UPDATE", "ZTABLE", "CHANGED_BY", (111, 114), "client_specified"),
            TestCase("T18", "UPDATE with calculations", "UPDATE", "ZTABLE", "UPDATED_BY", (116, 122), "calculations"),
            TestCase("T19", "UPDATE with CASE", "UPDATE", "ZTABLE", "PROCESSOR", (124, 132), "case_statement"),
            TestCase("T20", "UPDATE nested parentheses", "UPDATE", "ZTABLE", "CHANGED_BY", (134, 136), "nested_parens"),
            
            # MODIFY Tests
            TestCase("T21", "Basic MODIFY from structure", "MODIFY", "ZTABLE", "MODIFIED_BY", (142, 146), "basic"),
            TestCase("T22", "MODIFY FROM TABLE", "MODIFY", "ZTABLE", "MODIFIED_BY", (148, 154), "from_table"),
            TestCase("T23", "MODIFY with TRANSPORTING", "MODIFY", "ZTABLE", "MODIFIED_BY", (156, 159), "transporting"),
            TestCase("T24", "MODIFY internal table", "MODIFY", "LT_RECORDS", "PROCESSOR", (161, 162), "internal_table"),
            TestCase("T25", "MODIFY WHERE condition", "MODIFY", "LT_RECORDS", "PROCESSOR", (164, 167), "where_condition"),
            TestCase("T26", "MODIFY TABLE sorted", "MODIFY", "LT_SORTED", "MODIFIED_BY", (169, 173), "sorted_table"),
            TestCase("T27", "MODIFY from internal table", "MODIFY", "ZTABLE", "MODIFIED_BY", (175, 176), "from_itab"),
            TestCase("T28", "MODIFY VALUE constructor", "MODIFY", "ZTABLE", "MODIFIED_BY", (178, 182), "value_constructor"),
            TestCase("T29", "MODIFY inline work area", "MODIFY", "ZTABLE", "MODIFIED_BY", (184, 187), "inline"),
            TestCase("T30", "MODIFY LOOP field symbols", "MODIFY", "ZTABLE", "LAST_CHANGED_BY", (189, 194), "loop_fs"),
    ...
    def test_insert_operation(self, lines: List[str], test_case: TestCase) -> Dict:
        ...
        patterns = self.insert_handler.analyze_insert_values(code_snippet, start_idx)
        ...
        if enhanced_patterns:
            ...
            pattern = patterns[0]  # Take first pattern
            result['found'] = True
            result['table'] = pattern.table
            result['fields'] = list(pattern.fields.keys())
    ...
    def run_test(self, lines: List[str], test_case: TestCase) -> Dict:
        ...
        ops = self.comprehensive_handler.analyze_operations(lines, start_idx, end_idx)
        
        for op in ops:
            if op.operation == test_case.operation:
                return {
                    'found': True,
                    'table': op.table,
                    'fields': list(op.fields.keys()),
                    'has_sy_uname': any('sy-uname' in str(v).lower() for v in op.fields.values()),
                    'details': f"Pattern type: {op.pattern_type}, Confidence: {op.confidence}"
                }
...
def main():
    ...
    # Test file path
    test_file = "test/comprehensive_db_ops_test.abap"
